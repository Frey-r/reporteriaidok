...
with open('localdata.json','r') as archivo_json:
    data = json.load(archivo_json)

#DATOS DE CONECCIÓN-----------------------------------------------------------------------------------------------------
ip=data['conn']['ip']
puerto=data['conn']['puerto']
user=data['conn']['user']
password=data['conn']['password']
bd=data['conn']['bd']

#RUTAS------------------------------------------------------------------------------------------------------------------
excels = data['rutas']['excels']
resultados = data['rutas']['resultados']
...
def carga_groups():
    ...
    print(excels+'groups_members_count-(2023-10-02).csv')
...
def rutificado_FE(tabla,fechaInicio,fechaFinal):
    query='select _id,rut from users_sep '
    df = pd.read_sql_query(query,con=engine)
    Druts = {}
...
def informe_anual_corp():
    for i in range(9):
        ...
        # query = "SELECT e._id as 'ID Empresa',e.simple_name as 'Nombre empresa',sum(d.total_firmas) as 'Firmas ', count(d._id) as 'Documentos ' FROM empresas e left join documentos_all_sign d on e._id = d.enterprise_id where d.updated_at between '2022-0"+str(i+1)+"-01' and '2022-0"+str(i+1)+"-"+str(meses[i])+"' group by e._id,e.simple_name order by e._id asc;"
        # query = "SELECT d._id as 'ID Documento',u.rut as 'Rut Creador', d.enterprise_id as 'ID Empresa', d.updated_at as 'Fecha de Actualizacion',d.created_at as 'Fecha de Creacion' ,d.document_name as 'Nombre de Documento',d.signed_by_ids as 'Numero de Firmas' FROM documentos_all_sign d left join usuarios u on u._id = d.mxml_creator_id WHERE d.enterprise_id = '6182f5ac502d225be8008413' AND  d.updated_at between '2022-0"+str(i+1)+"-01' and '2022-0"+str(i+1)+"-"+str(meses[i])+"' order by d.updated_at asc;"
